...
export const PropertyRules = async ({ property }) => {
  ...
  const { property_policy, houseRules, petsPolicies } = propertyRules;

  console.log(property_policy);
  console.log(property_policy.houseRules.smokingAllowed.smokingAllowed);
      ...
      <div className="mt-8">
        ...
        <p className="mt-4 text-[18px] flex">
          ...
          {property_policy.petsPolicies.petsAllowed === "allowed"
            ? "allowed"
            : property_policy.petsPolicies.petsAllowed === "not_allowed" ? "not allowed" : "allowed with restrictions"}
          {property_policy.houseRules.smokingAllowed
            .nonSmokingRoomsAvailable === "true"
            ? "only at Smoking area"
            : ""}
